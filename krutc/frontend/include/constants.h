...
inline bool is_uppercase_keyword(std::string &k) {
  return k == Class || k == Inherits || k == For || k == Else || k == While || k == Return || k == Continue ||
         k == Break || k == New || k == Kill;
}
...
inline bool is_special_char(std::string &c) {
  return c == OpenP || c == CloseP || c == SemiColon || c == OpenBrack || c == CloseBrack || c == OpenBrace ||
         c == CloseBrace || c == Period || c == Comma || c == Colon;
}
...
inline bool is_binop(std::string &b) {
  return b == Plus || b == Minus || b == Times || b == Divide || b == LessThan || b == GreaterThan || b == LEQ ||
...
static std::unordered_map<TokenType, std::string> TOKEN_TYPE_TO_STR = {{CLASS, "CLASS"},
                                                                       {INHERITS, "INHERITS"},
                                                                       {FOR, "FOR"},
                                                                       {IF, "IF"},
                                                                       {ELSE, "ELSE"},
...
static std::unordered_map<std::string, TokenType> KEYWORDS_STR_TO_TTYPE{
    {"CLASS", CLASS},   {"INHERITS", INHERITS}, {"FOR", FOR},     {"IF", IF},     {"ELSE", ELSE}, {"WHILE", WHILE},
    {"RETURN", RETURN}, {"CONTINUE", CONTINUE}, {"BREAK", BREAK}, {"NONE", NONE}, {"NEW", NEW},   {"KILL", KILL}};
