...
class InheritanceGraph {
    ...
    void dfs(const std::string& node, std::map<std::string, bool>& visited, 
             std::map<std::string, bool>& recStack, std::vector<std::string>& path, 
    ...
    void check_for_cycles(std::vector<std::vector<std::string>>& cycles) {
        ...
        for (const auto& node : parents) {
            if (!visited[node.first]) {
                dfs(node.first, visited, recStack, path, cycles);
            }
        }
    }
};
